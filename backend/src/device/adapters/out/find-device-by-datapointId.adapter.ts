...
import {
  FIND_DEVICE_BY_DATAPOINTID_REPO_PORT,
  type FindDeviceByDatapointIdRepoPort,
} from 'src/device/application/repository/find-device-by-datapointId.repository';
...
export class FindDeviceByDatapointIdAdapter implements FindDeviceByDatapointIdPort {
  ...
  async findByDatapointId(cmd: FindDeviceByDatapointIdCmd): Promise<Device> {
    if (!cmd.datapointId)
      throw new Error('[FIND BY DATAPOINTID ADAPTER] There is no datapointId');

    const entity: DeviceEntity | null =
      await this.findByDatapointIdPort.findByDatapointId(cmd.datapointId);
    if (!entity)
      throw new Error(`Can't find device with datapoint ${cmd.datapointId}`);
