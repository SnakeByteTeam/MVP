...
describe('FindByDeviceIdAdapter', () => {
  ...
  it('should return a device array when succeed', async () => {
    ...
    expect(devices.map((d) => d.getName())).toEqual([
      'living-room-light',
      'kitchen-light',
    ]);
    expect(devices.map((d) => d.getPlantId())).toEqual([
      'plant-01',
      'plant-01',
    ]);
  ...
  it("should throw a Id is null error when command'id is absent", async () => {
    await expect(
      adapter.findByPlantId(null as unknown as FindDeviceByIdCmd),
    ).rejects.toThrow(Error('PlantId is null'));
  });

  it("should throw a Id is null error when command'id is absent", async () => {
    await expect(
      adapter.findByPlantId({ id: null } as unknown as FindDeviceByIdCmd),
    ).rejects.toThrow(Error('PlantId is null'));
  });

  it("should throw a Id is null error when command'id is absent", async () => {
    await expect(
      adapter.findByPlantId({ id: '' } as unknown as FindDeviceByIdCmd),
    ).rejects.toThrow(Error('PlantId is null'));
  });
  ...
  it("should return a Device not found error when the device can't be found", async () => {
    ...
    await expect(adapter.findByPlantId({ id: '123' })).rejects.toThrow(
      Error('Plant 123 not found'),
    );
  });
});
