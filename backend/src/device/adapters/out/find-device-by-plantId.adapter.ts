...
import { type FindDeviceByPlantIdRepoPort, FIND_DEVICE_BY_PLANTID_REPO_PORT } from "src/device/application/repository/find-device-by-plantId.repository";
...
export class FindDeviceByPlantIdAdapter implements FindDeviceByPlantIdPort{
    ...
    constructor(
        ...
        @Inject(FIND_DEVICE_BY_PLANTID_REPO_PORT) private readonly findDeviceRepo: FindDeviceByPlantIdRepoPort
    ) {}
    ...
    async findByPlantId(cmd: FindDeviceByPlantIdCmd): Promise<Device[]> {
        ...
        const deviceEntities: DeviceEntity[] | null = await this.findDeviceRepo.findByPlantId(plantId);
        
        if(!deviceEntities) throw(new Error(`Plant ${plantId} not found`));

        const devices: Device[] = deviceEntities.map((entity) => this.mapper.toDomain(entity))
        return devices;
    }
