...
import {
  WRITE_DATAPOINT_VALUE_REPO_PORT,
  type WriteDatapointValueRepoPort,
} from 'src/device/application/repository/write-datapoint-value.repo';
...
export class WriteDatapointValueAdapter implements WriteDatapointValuePort {
  ...
  async writeDatapointValue(cmd: WriteDatapointValueCmd): Promise<void> {
    ...
    const result: boolean = await this.writeDatapointRepoPort.writeDeviceValue(
      token,
      cmd.plantId,
      cmd.datapointId,
      cmd.value,
