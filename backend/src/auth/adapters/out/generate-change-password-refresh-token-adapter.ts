...
export class GenerateChangePasswordRefreshTokenAdapter implements GenerateChangePasswordRefreshTokenPort {
    ...
    constructor(
        @Inject(JWT_CHANGE_PASSWORD_REFRESH_TOKEN_GENERATOR) private readonly jwtChangePasswordRefreshTokenGenerator: JwtChangePasswordRefreshTokenGenerator
    ){}

    generateChangePasswordRefreshToken(req: GenerateChangePasswordRefreshTokenCmd): string {
        return this.jwtChangePasswordRefreshTokenGenerator.generateChangePasswordRefreshToken(req.payload);
    }
}
