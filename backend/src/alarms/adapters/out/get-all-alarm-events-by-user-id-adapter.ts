...
import { GET_ALL_ALARM_EVENTS_BY_USER_ID_REPOSITORY, GetAllAlarmEventsByUserIdRepository } from '../../application/repository/get-all-alarm-events-by-user-id-repository.interface';

export class GetAllAlarmEventsByUserIdAdapter implements GetAllAlarmEventsByUserIdPort {

    constructor(
        @Inject(GET_ALL_ALARM_EVENTS_BY_USER_ID_REPOSITORY) private readonly getAllAlarmEventsByUserIdRepository: GetAllAlarmEventsByUserIdRepository
    ){}

  async getAllAlarmEventsByUserId(
    req: GetAllAlarmEventsByUserIdCmd,
  ): Promise<AlarmEvent[]> {
    const alarmEvents = await this.getAllAlarmEventsByUserIdRepository.getAllAlarmEventsByUserId(req.id);

    return alarmEvents.map(alarmEvent => new AlarmEvent(
        alarmEvent.id,
