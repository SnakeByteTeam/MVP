...
describe('DeviceController', () => {
    ...
    it('should return device dto when requested by id, correctly converted into a dto', async () => {
        const returnedDatapoints: Datapoint[] = [new Datapoint('456', 'topolino', true, false, 'number', ['on', 'off'], 'ciao')];
        const returnedDevice: Device = new Device('123', '123', 'pippo', 'luce', 'luce', returnedDatapoints);
        ...
        const expectedDatapointsDto: DatapointDto[] = returnedDatapoints.map((datapoint) => ({
            id: datapoint.getId(),
    ...
    it('should return all plant\'s devices when requested by plant id, correctly converted into a dto', async () => {
        const returnedDatapointsA: Datapoint[] = [
            new Datapoint('456', 'brightness', true, true, 'number', ['0', '100'], 'slider'),
            new Datapoint('457', 'power', true, true, 'boolean', ['on', 'off'], 'switch'),
        ];
        const returnedDatapointsB: Datapoint[] = [
            new Datapoint('458', 'temperature', true, false, 'number', ['16', '30'], 'sensor'),
        ];
        const returnedDatapointsC: Datapoint[] = [
            new Datapoint('459', 'mode', true, true, 'string', ['cool', 'heat', 'dry'], 'select'),
            new Datapoint('460', 'fanSpeed', true, true, 'string', ['low', 'mid', 'high'], 'select'),
            new Datapoint('461', 'lock', true, true, 'boolean', ['locked', 'unlocked'], 'toggle'),
        ];

        const returnedDevices: Device[] = [
            new Device('123', 'plant-01', 'living-room-light', 'light', 'dimmer', returnedDatapointsA),
            new Device('124', 'plant-01', 'bedroom-thermostat', 'climate', 'thermostat', returnedDatapointsB),
            new Device('125', 'plant-01', 'kitchen-ac', 'climate', 'air-conditioner', returnedDatapointsC),
        ];
        ...
        expect(findDeviceByPlantId.findByPlantId).toHaveBeenCalledWith({ id: 'plant-01' });
    ...
    it('should throw an InternalServerErrorException when catch an error', async () => {
        ...
        await expect(controller.findById).rejects.toThrow(InternalServerErrorException);
    });

    it('should throw an InternalServerErrorException when catch an error', async () => {
        findDeviceByPlantId.findByPlantId.mockRejectedValue(new Error('Error'));

        await expect(controller.findByPlantId).rejects.toThrow(InternalServerErrorException);
    });
});