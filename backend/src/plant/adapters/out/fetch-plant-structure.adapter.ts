...
export class FetchPlantStructureAdapter implements FetchPlantStructurePort {
  constructor(
    ...
    @Inject(FETCH_PLANT_STRUCTURE_REPO_PORT)
    private readonly fetchPlantRepoPort: FetchPlantStructureRepo,
  ) {}
  ...
  async fetch(plantId: string): Promise<Plant> {
    ...
    const plantDto: PlantDto | null = await this.fetchPlantRepoPort.fetch(
      token,
      plantId,
    );
