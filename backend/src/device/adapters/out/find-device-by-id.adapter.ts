...
export class FindDeviceByIdAdapter implements FindDeviceByIdPort {
  constructor(
    ...
    private readonly findDeviceByIdRepository: FindDeviceByIdRepoPort,
  ) {}
  ...
  async findById(cmd: FindDeviceByIdCmd): Promise<Device> {
    ...
    const deviceEntity: DeviceEntity | null =
      await this.findDeviceByIdRepository.findById(deviceId);
