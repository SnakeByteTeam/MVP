...
import { READ_CACHE_REPO_PORT, type ReadCacheRepoPort } from "src/cache/application/repository/read-cache.repository";
...
export class ReadCacheAdapter implements ReadCachePort {
    ...
    constructor(
        @Inject(READ_CACHE_REPO_PORT) private readonly readCacheRepo: ReadCacheRepoPort
    ) {}

    async readCache(cmd: ReadCacheCmd): Promise<Plant | null> {
        if(!cmd?.plantId) throw new Error('PlantId is null');

        const plantEntity: PlantEntity | null = await this.readCacheRepo.read(cmd.plantId);
