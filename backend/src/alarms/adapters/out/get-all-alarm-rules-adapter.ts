...
import { GET_ALL_ALARM_RULES_REPOSITORY, GetAllAlarmRulesRepository } from '../../application/repository/get-all-alarm-rules-repository.interface';

export class GetAllAlarmRulesAdapter implements GetAllAlarmRulesPort {

    constructor(
        @Inject(GET_ALL_ALARM_RULES_REPOSITORY) private readonly getAllAlarmRulesRepository: GetAllAlarmRulesRepository
    ){}

  async getAllAlarmRules(): Promise<AlarmRule[]> {
    const alarmRules = await this.getAllAlarmRulesRepository.getAllAlarmRules();

    return alarmRules.map((alarmRule) => new AlarmRule(
        alarmRule.id,
        alarmRule.name,
