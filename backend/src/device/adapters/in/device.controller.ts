import { Controller, Query, Get, Inject, InternalServerErrorException } from "@nestjs/common";
...
import { type FindDeviceByIdUseCase, FIND_DEVICE_BY_ID_USECASE} from "src/device/application/ports/in/find-device-by-id.usecase";
import { type FindDeviceByPlantIdUseCase, FIND_DEVICE_BY_PLANTID_USECASE } from "src/device/application/ports/in/find-device-by-plantid.usecase";
...
export class DeviceController{
    ...
    constructor(
        @Inject(FIND_DEVICE_BY_ID_USECASE) private readonly findByIdUseCase: FindDeviceByIdUseCase, 
        @Inject(FIND_DEVICE_BY_PLANTID_USECASE) private readonly findByPlantIdUseCase: FindDeviceByPlantIdUseCase
    ) {}
    ...
    async findByPlantId(@Query('plantId') plantId: string): Promise<DeviceDto[]> {
        ...
        try{
            const devices: Device[] = await this.findByPlantIdUseCase.findByPlantId(findByPlantIdCmd);
    ...
    private deviceToDto(device: Device): DeviceDto {
        const datapointsDto: DatapointDto[] = device.getDatapoints().map((dp) => this.datapointToDto(dp));
