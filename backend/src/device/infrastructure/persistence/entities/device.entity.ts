...
export class DeviceEntity {
  ...
  static toDomain(entity: DeviceEntity): Device {
    const datapoints = entity.datapoints.map((datapoint) =>
      DatapointEntity.toDomain(datapoint),
    );
    return new Device(
      entity.id,
      entity.plantId,
  ...
  static fromDomain(device: Device): DeviceEntity {
    ...
    entity.datapoints = device
      .getDatapoints()
      .map((datapoint) => DatapointEntity.fromDomain(datapoint));
    return entity;
  }
}
