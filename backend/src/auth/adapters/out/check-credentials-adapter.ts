...
import { CHECK_CREDENTIALS_REPOSITORY, CheckCredentialsRepository } from "../../application/repository/check-credentials-repository.interface";
...
export class CheckCredentialsAdapter implements CheckCredentialsPort {
    ...
    constructor(
        @Inject(CHECK_CREDENTIALS_REPOSITORY) private readonly checkCredentialsRepository: CheckCredentialsRepository
    ){}

    async checkCredentials(req: CheckCredentialsCmd): Promise<Payload> {
        const credentials: PayloadEntity = await this.checkCredentialsRepository.checkCredentials(req.username, req.password);
        return new Payload(
            credentials.id,
            credentials.role
        );
