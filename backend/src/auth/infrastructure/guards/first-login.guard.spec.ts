...
describe('FirstLoginGuard', () => {
  ...
  it('allows when Authorization Bearer token has payload.firstAccess === true', () => {
    ...
    (guard as any).jwtService = { verify: jest.fn().mockReturnValue({ firstAccess: true }) };
    ...
    expect((guard as any).jwtService.verify).toHaveBeenCalledWith('token', { secret: process.env.ACCESS_SECRET });
  });
  ...
  it('throws UnauthorizedException when Authorization header is malformed', () => {
    ...
    const ctx = {
      switchToHttp: () => ({ getRequest: () => ({ headers: { authorization: 'BadHeader' } }) }),
    } as unknown as ExecutionContext;
  ...
  it('throws UnauthorizedException when token is invalid (verify throws)', () => {
    const guard = new FirstLoginGuard();
    (guard as any).jwtService = { verify: jest.fn().mockImplementation(() => { throw new Error('invalid'); }) };

    const ctx = {
      switchToHttp: () => ({ getRequest: () => ({ headers: { authorization: 'Bearer bad' } }) }),
    } as unknown as ExecutionContext;
  ...
  it('throws UnauthorizedException when payload.firstAccess !== true', () => {
    const guard = new FirstLoginGuard();
    (guard as any).jwtService = { verify: jest.fn().mockReturnValue({ firstAccess: false }) };
