...
export class GetAlarmRuleByIdAdapter implements GetAlarmRuleByIdPort {
    ...
    constructor(
        @Inject(GET_ALARM_RULE_BY_ID_REPOSITORY) private readonly getAlarmRuleByIdRepository: GetAlarmRuleByIdRepository 
    ){}

  async getAlarmRuleById(req: GetAlarmRuleByIdCmd): Promise<AlarmRule | null> {
    const alarmRule = await this.getAlarmRuleByIdRepository.getAlarmRuleById(req.id);
