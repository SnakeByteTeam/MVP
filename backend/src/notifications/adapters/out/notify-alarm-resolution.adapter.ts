...
import {
  ...
  type NotifyAlarmResolutionRepoPort,
} from 'src/notifications/application/repository/notify-alarm-resolution.repository';
...
export class NotifyAlarmResolutionAdapter implements NotifyAlarmResolutionPort {
  ...
  async notifyAlarmResolution(cmd: NotifyAlarmResolutionCmd): Promise<void> {
    await this.notifyResolutionRepo.notifyAlarmResolution(
      cmd.alarmId,
      cmd.wardId,
    );
  }
