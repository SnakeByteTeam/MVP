...
export class ChangeCredentialsAdapter implements ChangeCredentialsPort {
  constructor(
    @Inject('CHANGE_CREDENTIALS_REPOSITORY')
    private readonly changeCredentialsRepository: ChangeCredentialsRepository,
  ) {}

  changeCredentials(req: ChangeCredentialsCmd): Promise<void> {
    return this.changeCredentialsRepository.changeCredentials(
      req.username,
      req.newPassword,
      req.firstAccess,
    );
