...
describe('FindByDeviceIdAdapter', () => {
  ...
  it("should throw a Id is null error when command'id is absent", async () => {
    await expect(
      adapter.findById(null as unknown as FindDeviceByIdCmd),
    ).rejects.toThrow(Error('Id is null'));
  });

  it("should throw a Id is null error when command'id is absent", async () => {
    await expect(
      adapter.findById({ id: null } as unknown as FindDeviceByIdCmd),
    ).rejects.toThrow(Error('Id is null'));
  });

  it("should throw a Id is null error when command'id is absent", async () => {
    await expect(
      adapter.findById({ id: '' } as unknown as FindDeviceByIdCmd),
    ).rejects.toThrow(Error('Id is null'));
  });
  ...
  it("should return a Device not found error when the device can't be found", async () => {
    ...
    await expect(adapter.findById({ id: '123' })).rejects.toThrow(
      Error('Device 123 not found'),
    );
  });
});
