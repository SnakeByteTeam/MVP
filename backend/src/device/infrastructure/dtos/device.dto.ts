...
export class DeviceDto {
    ...
    static toDomain(dto: DeviceDto): Device {
        const datapoints = dto.datapoints.map((datapoint) => DatapointDto.toDomain(datapoint));
        return new Device(
            dto.id,
            dto.plantId,
            dto.name,
    ...
    static fromDomain(device: Device): DeviceDto {
        ...
        dto.datapoints = device.getDatapoints().map((datapoint) => DatapointDto.fromDomain(datapoint));
        return dto;
    }
}